...
def getQuorum():
    ...
    if(totalCredits < token):
        while(len(quorumlist) < 7):
            ...
            for record in dataTable:
                if (record['didHash'] == newQuorum[temp]):
                    nodePeerId= record['peerid']
                    break

            url = "http://localhost:8088/node-status?peerId={}&role=alpha".format(nodePeerId)
            res = requests.get(url)
            nodeStatus = res.text
            nodePeerId= None    

            if(not(newQuorum[temp]['didHash'] in quorumlist) and not(newQuorum[temp]['didHash'] == sender) and not(newQuorum[temp]['didHash'] == receiver) and newQuorum[temp]['alpha'] == 0 and nodeStatus == 'true'):
        ...
        while(flag):
            ...
            while(count < alphaSize):
                ...
                i = random.randint(0, index)
                  #TODO do a curl here to nidhin's api after doing a readfile of local dataTable and fetch the did's corresponding peerid ,then get true or false back as reply after parsing then add that true/false variable with "and" operation to the following statement to check if node is pingable
                for record in dataTable:
                    if (record['didHash'] == newQuorum[i]['didHash']):
                        nodePeerId= record['peerid']
                        break
                ...
                if(newQuorum[i]['credits'] >= minValue and newQuorum[i]['alpha'] == 0 and not(newQuorum[i]['didHash'] == sender) and not(newQuorum[i]['didHash'] == receiver) and not(i in alphaListIndices) and nodeStatus == 'true'):
                    count += 1
    ...
    while(len(quorumlist) < count+7):
        ...
        url = "http://localhost:8088/node-status?peerId={}&role=beta".format(nodePeerId)
        res = requests.get(url)
        nodeStatus = res.text
        nodePeerId= None

        if(not(newQuorum[temp]['didHash'] in quorumlist) and not(newQuorum[temp]['didHash'] == sender) and not(newQuorum[temp]['didHash'] == receiver) and newQuorum[temp]['beta'] == 0 and nodeStatus == 'true'):
            quorumlist.append(newQuorum[temp]['didHash'])
    ...
    while(len(quorumlist) < count+14):
        ...
        url = "http://localhost:8088/node-status?peerId={}&role=gamma".format(nodePeerId)
        res = requests.get(url)
        nodeStatus = res.text
        nodePeerId= None

        if(not(newQuorum[temp]['didHash'] in quorumlist) and not(newQuorum[temp]['didHash'] == sender) and not(newQuorum[temp]['didHash'] == receiver) and newQuorum[temp]['gamma'] == 0 and nodeStatus == 'true'):
            quorumlist.append(newQuorum[temp]['didHash'])
