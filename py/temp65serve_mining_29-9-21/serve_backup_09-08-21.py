    ...
    if(totalCredits < token):
        ...
        while(flag):
            ...
            while(count < alphaSize):
                if(newQuorum[i]['credits'] >= minValue and newQuorum[i]['alpha'] == 0 and not(newQuorum[i]['didHash'] == sender) and not(newQuorum[i]['didHash'] == receiver)):
                    count += 1
        ...
        for i in alphaListIndices:
            ...
            # sum = 0
            # while(sum<token or len(alphaListIndices)<7):
            #    temp = random.randint(0,len(newQuorum)-1)
            #    if(not(temp in alphaListIndices)and not(newQuorum[temp]['didHash']==sender) and not(newQuorum[temp]['didHash']==receiver) and newQuorum[temp]['alpha'] == 0):
            #       alphaListIndices.append(temp)
            #       sum += newQuorum[temp]['credits']
            #    if(len(alphaListIndices)>7 and sum<token):
            #       alphaListIndices = []
